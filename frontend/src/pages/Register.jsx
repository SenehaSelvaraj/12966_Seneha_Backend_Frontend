...
function Register() {
  ...
  const validatePassword = (password) => {
    ...
    if (!/[@_]/.test(password))
      rules.push('At least one special character (@ or _)');
    if (/[^a-zA-Z0-9@_]/.test(password))
      rules.push('Only @ and _ are allowed as special characters');
    return rules;
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h5 style={{ fontWeight: '700', marginBottom: '4px' }}>Create Account</h5>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <input
              style={inputStyle(errors.password?.length > 0)}
              name="password"
              type="password"
              value={form.password}
              onChange={handleChange}
        ...
        <div style={{ borderTop: '1px solid #dee2e6', marginTop: '20px', paddingTop: '16px', textAlign: 'center', fontSize: '13px' }}>
          Already have an account? <a href="/" style={{ color: '#212529', fontWeight: '600' }}>Login</a>
        </div>
      </div>
    </div>
  );
