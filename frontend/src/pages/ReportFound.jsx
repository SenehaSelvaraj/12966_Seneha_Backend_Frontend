...
function ReportFound() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await itemApi.post('/api/items/report-found', { ...form, reportedBy: userId });
      setMessage('Found item reported! Admin will review it.');
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8f9fa', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'white', borderRadius: '8px', padding: '32px', width: '480px', boxShadow: '0 2px 12px rgba(0,0,0,0.1)' }}>
        <div style={{ borderLeft: '4px solid #198754', paddingLeft: '12px', marginBottom: '24px' }}>
        ...
        <form onSubmit={handleSubmit}>
          {[
            { label: 'Item Name', name: 'itemName', placeholder: 'e.g. Black Wallet' },
            { label: 'Location Found', name: 'location', placeholder: 'e.g. Library 2nd Floor' }
          ].map(f => (
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <textarea style={{ ...inputStyle, resize: 'vertical' }} name="description"
              ...
              placeholder="Describe the item — color, brand, contents..." />
          </div>
 
          <button type="submit" style={{ width: '100%', padding: '9px', background: '#198754', color: 'white', border: 'none', borderRadius: '4px', fontSize: '14px', cursor: 'pointer', fontWeight: '600' }}>
            Submit Found Report
          </button>
        </form>
      </div>
